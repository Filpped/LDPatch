#!/usr/bin/env python3
"""
对Fedora和openEuler-24.03共同软件包进行补丁比对（适配strip_level），并写入日志文件。
"""
...
console.setLevel(logging.WARNING)  # 控制台只输出 warning 及以上
...
def get_spec_content(package_name, distribution, spec_dir=DEFAULT_SPEC_DIR):
    ...
    try:
        ...
        logging.error(f"获取spec文件失败: {package_name}({distribution}): {e}")
        return None
...
def main(package_analysis_file, output_file="rpm_patch_comparison_report.json"):
    ...
    for idx, (package_key, package_info) in enumerate(common_packages.items(), 1):
        try:
            logging.info(f"[{idx}/{total_pkgs}] 处理软件包: {package_key}")
            ...
            if not fedora_spec_content or not openeuler_spec_content:
                ...
                logging.warning(f"spec文件缺失: {package_key}")
                continue
            ...
            for patch_name in fedora_patches:
                ...
                if content:
                    ...
                    logging.warning(f"Fedora补丁内容缺失: {patch_name}")
            for patch_name in openeuler_patches:
                ...
                if content:
                    ...
                    logging.warning(f"openEuler补丁内容缺失: {patch_name}")
            matched_fedora = set()
            ...
            for fname in list(fedora_left):
                ...
                for oname in list(openeuler_left):
                    ...
                    if sim == 1.0:
                        ...
                        logging.info(f"[完全相同补丁] Fedora: {fname}, openEuler: {oname}")
                        found = True
                        break
                if found:
                    continue

            # --- 剩余补丁，判定相似度大于阈值（功能相似但内容不同） ---
            ...
            for fname in list(fedora_left):
                ...
                for oname in list(openeuler_left):
                    ...
                    if is_sim:
                        ...
                        matched_fedora.add(fname)
                        matched_openeuler.add(oname)
                        logging.info(f"[功能相似补丁] Fedora: {fname}, openEuler: {oname}, similarity={sim:.3f}")
                        break
            ...
            if unique_fedora:
                logging.info(f"Fedora独有补丁: {unique_fedora}")
            if unique_openeuler:
                logging.info(f"openEuler独有补丁: {unique_openeuler}")
            ...
            logging.error(f"处理包时发生异常: {package_key}，错误信息: {e}")
    ...
    summary = f"\n========== 比对摘要 =========="
    summary += f"\n共同补丁总数: {total_common}"
    summary += f"\n相似补丁总数: {total_similar}"
    summary += f"\nFedora独有补丁总数: {total_fedora_unique}"
    summary += f"\nOpenEuler独有补丁总数: {total_openeuler_unique}"
    summary += f"\nFedora缺失补丁总数: {total_missing_fedora}"
    summary += f"\nOpenEuler缺失补丁总数: {total_missing_openeuler}"
    summary += f"\n=============================\n"
